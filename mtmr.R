...
power_simulation <- function(m, n, h, r, sigma_r, cov_coef, mu, all, level=0.95){
  ...
  delta <- true_theta_hat_h[1] - true_theta_hat_h[2]  #two methods are different in fact
...
print(mean(auc_1_record-auc_2_record))
qqnorm(diff_record) ##test normality

empSE <- sqrt(var(diff_record))
estSE <- bootstrap_result[3]
power <- pnorm(z_norm + delta/estSE , mean = 0, sd = 1, lower.tail = TRUE, log.p = FALSE)+
  pnorm(z_norm - delta/estSE , mean = 0, sd = 1, lower.tail = TRUE, log.p = FALSE)

#judge whether the influence of readers is under control 
make_decision <- function(x_rate, y_rate){
  w_11 <- rep(1/m, m)
  w_12 <- matrix(rep(1/r, r), ncol=1)
  a11 <- var(c(w_11 %*% x_rate[,1:r]))
  a12 <- var(c(x_rate[,1:r] %*% w_12))
  b11 <- var(c(w_11 %*% x_rate[,(r+1):(2*r)]))
  b12 <- var(c(x_rate[,(r+1):(2*r)] %*% w_12))
  w_21 <- rep(1/n, n)
  w_22 <- matrix(rep(1/r, r), ncol=1)
  a21 <- var(c(w_21 %*% y_rate[,1:r]))
  a22 <- var(c(y_rate[,1:r] %*% w_22))
  b21 <- var(c(w_21 %*% y_rate[,(r+1):(2*r)]))
  b22 <- var(c(y_rate[,(r+1):(2*r)] %*% w_22))
  cov_reader_a <- cov(c(w_11 %*% x_rate[,1:r]), c(w_21 %*% y_rate[,1:r]))
  cov_reader_b <- cov(c(w_11 %*% x_rate[,(r+1):(2*r)]),c(w_21 %*% y_rate[,(r+1):(2*r)]))
  if ((a11+a21-2*cov_reader_a)/(a12+a22) > 0.1  | (b11+b21-2*cov_reader_b)/(b12+b22) > 0.1){return('bootstrap')}
  else{return('simple')}
}
